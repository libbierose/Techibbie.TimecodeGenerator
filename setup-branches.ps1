...
#   4. Switches back to 'dev' as the active branch.
...
if ($currentBranch -ne 'dev') {
    Write-Host "Renaming branch '$currentBranch' to 'dev'..."
    git branch -m $currentBranch dev
} else {
    Write-Host "Already on 'dev'."
}

# Step 2: create release branch from dev if it does not exist
$existing = git branch --list release
if ($existing) {
...
# Step 3: push both branches to origin
...
Write-Host ""
Write-Host "Recommended: go to GitHub Settings -> Branches and set 'dev' as the default branch."
